...
public class Solution_test_2105_디저트카페 {
	...
	private static void dfs(int firstR, int firstC, int r, int c, int dir, int cnt) {
		if(dir==4) {
			if(firstR==r && firstC==c) count=Math.max(count, cnt);
			return;
		}
		...
		if(nr>=0 && nr<N && nc>=0 && nc<N && !dessert[map[nr][nc]]) {
			...
			dfs(firstR, firstC, nr, nc, dir, cnt+1);
			dfs(firstR, firstC, nr, nc, dir+1, cnt+1);
